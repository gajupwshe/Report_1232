                                    ...
                                    <center>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control " name="valve_serial_no" style="margin: 20px; width: 200px;">
                                                <option value="All">All</option>
                                                <?php
                                                $drop = "SELECT * FROM `valve_data` GROUP BY vsn";
                                                $result = $conn->query($drop);
                                                ...
                                                if ($result->num_rows > 0) {
                                                    $row = $result->fetch_assoc();
                                                    do { ?>
                                                        <option value="<?php echo $row["vsn"];?>"><?php echo $row["vsn"];?></option>
                                                        <?php
                                                    } while ($row = $result->fetch_assoc());
                                                }
                                                ?>
...
//                                            echo "Your favorite color is red!";
                                    ...
                                    //                        echo '<br/>date after adding 1 day: ' . $new_end_date;
                                            ...
                                            if ($new_start_date >= $new_end_date && $new_end_date >= $current_date) {
                                                ...
                                                if ($test_type=="All"&&$valve_class=="All"&&$valve_size=="All"&&$valve_type=="All"&&$customer_name=="All"&&$valve_serial_no=="All"){
                                                    ...
                                                    $sql = "SELECT ti.*,tr.`test_result`,tr.`mt` ,tr.`pressure_unit` FROM `valve_data` ti, test_result tr  WHERE ti.test_no = tr.test_no AND  ti.test_type = tr.test_type AND  ti.`date` BETWEEN '$new_start_date' AND '$new_end_date'";
           // echo "all are all ".$sql;


                                                }
                                                else{
                                                    $sql = "SELECT ti.`date`,ti.`test_no`,ti.`vsn`,ti.`holding_set`,ti.`test_type`,ti.`hydro_set_pressure`,tr.`test_result` FROM `initial_valve_data_unit1` ti JOIN `test_result_unit1` tr ON (tr.`test_no` = ti.`test_no` AND ti.`test_type` = tr.`test_type`) WHERE ti.`date` BETWEEN '$new_start_date' AND '$new_end_date' AND$test_type$valve_class$valve_size$valve_type$customer_name$valve_serial_no" ;
       ...
       // echo "all are all ".$sql;
                                    ...
                                    $sql = "SELECT ti.*,tr.`test_result`,tr.`mt` ,tr.`pressure_unit` FROM `valve_data` ti, test_result tr  WHERE ti.test_no = tr.test_no AND  ti.test_type = tr.test_type AND  ti.`date`  BETWEEN '$new_start_date' AND '$new_end_date' AND$test_type$valve_class$valve_size$valve_type$customer_name$valve_serial_no" ;
